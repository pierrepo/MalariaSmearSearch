...
outputtablenames=$(sqlite3 data.db '.tables')
...
for tablename in "${tablenames[@]}"; do
    echo $tablename ;
    sqlite3 -header -csv data.db "select * from $tablename;" > $tablename.csv
done
...
while read line; do
    ...
    smeartype_licence_magnification=$(grep "^${sampleid}," Samples.csv | cut -f 3,7,9 -d "${DELIMITER}");
    ...
    echo $processed_annotation >> "extracted_data_$now.csv"
